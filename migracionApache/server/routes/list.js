...
const CAT_PALETTE = [
  { c:'#2563eb', bg:'#eff6ff', i:'🏷️' }, { c:'#0891b2', bg:'#ecfeff', i:'🏷️' },
  { c:'#059669', bg:'#ecfdf5', i:'🏷️' }, { c:'#d97706', bg:'#fffbeb', i:'🏷️' },
  { c:'#7c3aed', bg:'#f5f3ff', i:'🏷️' }, { c:'#db2777', bg:'#fdf2f8', i:'🏷️' },
...
function mergeCats(savedCats, items) {
  ...
  const missingNames = [...inventoryNames.entries()]
    ...
    .map(([, name]) => name).sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }));
...
function mergeProfesores(profesoresRows, usuariosRows) {
  ...
  for (const u of usuariosRows || []) {
    ...
    profesores.push({ id: 'user:' + u.usuario, nombre, departamento: 'Usuarios app', email: u.email || '', source: 'usuarios', usuario: u.usuario });
...
router.get('/', async (req, res) => {
  ...
  if (!catRenameMigrated) {
    ...
    const oldRows = rows.filter(r => { const key = normText(r.name); return key === 'consumible' || key === 'consumibles'; });
    const newRow = rows.find(r => normText(r.name) === normText(CAT_NEW));
    const seed = newRow || oldRows[0] || null;
    await DB.prepare("UPDATE inventario SET cat=? WHERE lower(trim(cat)) IN ('consumible','consumibles')").bind(CAT_NEW).run().catch(() => {});
    if (newRow || oldRows.length) {
      await DB.prepare("DELETE FROM categorias WHERE lower(trim(name)) IN ('consumible','consumibles') OR lower(trim(name))='material de taller'").run().catch(() => {});
      await DB.prepare('INSERT INTO categorias (name,c,bg,i,orden) VALUES (?,?,?,?,?)')
  ...
  const [items, profesores, usuarios, prestamos, aulas, cats, ciclosRows] = await Promise.all([
    DB.prepare(itemsQuery).all(),
    DB.prepare("SELECT * FROM profesores WHERE nombre != '' AND lower(nombre) != 'departamento' ORDER BY nombre").all(),
    DB.prepare("SELECT usuario, nombre, email FROM usuarios WHERE nombre != '' ORDER BY nombre").all(),
    DB.prepare('SELECT * FROM prestamos ORDER BY id').all(),
    DB.prepare('SELECT * FROM aulas ORDER BY orden').all(),
    DB.prepare('SELECT * FROM categorias ORDER BY orden').all(),
    DB.prepare('SELECT * FROM ciclos ORDER BY cicloOrden, modOrden').all(),
  ]);
  ...
  res.json({
    ok: true, itemsH: HEADERS_INV, itemsC,
    profesores: mergeProfesores(profesores.results, usuarios.results),
    prestamos: prestamos.results, aulas: aulas.results,
    cats: mergeCats(cats.results, itemRows),
    ciclos: cicloOrder.map(id => cicloMap[id]), user,
  });
});
