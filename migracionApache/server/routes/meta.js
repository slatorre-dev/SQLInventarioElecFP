...
function mergeCats(savedCats, inventoryCats) {
  ...
  const missing = (inventoryCats || [])
    .map(c => String(c.cat || c.name || '').trim()).filter(Boolean)
...
function mergeUbicaciones(savedRows, inventoryRows) {
  ...
  const missing = (inventoryRows || [])
    .map(r => String(r.loc || r.name || '').trim()).filter(Boolean)
    .filter(name => { const key = name.toLowerCase(); if (seen.has(key)) return false; seen.add(key); return true; })
    .sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }))
...
router.get('/', async (req, res) => {
  ...
  const [aulas, cats, invCats, ubicaciones, invLocs, ciclosRows] = await Promise.all([
    ...
    DB.prepare("SELECT DISTINCT cat FROM inventario WHERE cat IS NOT NULL AND trim(cat) != '' ORDER BY cat").all(),
    DB.prepare('SELECT * FROM ubicaciones ORDER BY orden, name').all().catch(() => ({ results: [] })),
    DB.prepare("SELECT DISTINCT loc FROM inventario WHERE loc IS NOT NULL AND trim(loc) != '' ORDER BY loc").all(),
